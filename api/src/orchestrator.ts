...
import {
  ...
  getAppPublicUrl,
  waitForOperation,
} from "./cloudways/apps.js";
import { purgeDeploymentCaches } from "./cloudways/cache.js";
import {
  startGitClone,
  startGitPull,
  type StartGitCloneResponse,
} from "./cloudways/git.js";
import { getAccessToken } from "./cloudways/auth.js";
import type {
  CloudwaysOperationKind,
  CloudwaysOperationRef,
  DeployRequest,
} from "./types.js";
...
export async function runDeployment(
  ...
  const recordCloudwaysOp = (
    kind: CloudwaysOperationKind,
    operationId: string,
  ): CloudwaysOperationRef => {
    const ref: CloudwaysOperationRef = {
      kind,
      operationId,
      startedAt: Date.now(),
    };
  ...
  const updateCloudwaysOpStatus = (operationId: string, status: string): void => {
    const current = store.get(deploymentId);
    const existing = current?.cloudwaysOperations;
    if (!existing) return;
    const next = existing.map((ref) =>
      ref.operationId === operationId ? { ...ref, status } : ref,
    );
  ...
  try {
    ...
    const detail =
      err instanceof CloudwaysApiError
