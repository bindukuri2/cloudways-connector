...
export async function listServers(client: CloudwaysClient): Promise<ServerSummary[]> {
  const body = await client.request<Record<string, unknown>>("/server");
  const raw = pickArray(body, ["servers"]) ?? [];
  return raw
    .map((r) => normalizeServer(r as Record<string, unknown>))
    .filter((s): s is ServerSummary => s !== null);
}
...
export async function getServer(
  ...
  const raw = (body.server ?? body) as Record<string, unknown>;
 ...
 */
export async function findServerIdByLabel(
  client: CloudwaysClient,
  serverLabel: string,
...
export async function listProviders(client: CloudwaysClient): Promise<Provider[]> {
  const body = await client.request<Record<string, unknown>>("/providers");
  ...
  return raw
    .map((r) => {
      ...
      return { code, name } satisfies Provider;
    })
...
export async function listRegions(
  ...
  const body = await client.request<Record<string, unknown>>("/regions");
  const container = (body.regions ?? body) as unknown;

  // Cloudways returns either a flat array (older) or a { cloud: [...] } map (newer).
  let raw: unknown[] = [];
  if (Array.isArray(container)) {
    raw = container as unknown[];
  } else if (container && typeof container === "object") {
    const map = container as Record<string, unknown>;
    const bucket = map[cloud];
    if (Array.isArray(bucket)) raw = bucket;
  }
  ...
  for (const r of raw) {
    ...
    if (objCloud && objCloud !== cloud && !Array.isArray(container)) continue;
...
export async function listInstanceSizes(
  ...
  const body = await client.request<Record<string, unknown>>("/server_sizes");
  const container = (body.sizes ?? body.server_sizes ?? body) as unknown;
...
export async function listAppVersions(
  ...
  const body = await client.request<Record<string, unknown>>("/app_version");
  const container = (body.app_versions ?? body.versions ?? body) as unknown;
  ...
  if (Array.isArray(container)) {
    ...
    if (Array.isArray(bucket)) raw = bucket;
      ...
      for (const [version, meta] of Object.entries(bucket as Record<string, unknown>)) {
  ...
  for (const r of raw) {
    ...
    if (!version) continue;
    const isDefault =
      obj.is_default === true || obj.is_default === 1 || obj.is_default === "1";
    out.push({ application, version, isDefault });
  }
  return out;
}

export interface CreateServerRawResponse {
  operation_id: string;
  server_id?: string;
  raw: unknown;
}
...
export async function createServer(
...
): Promise<CreateServerRawResponse> {
  const body = await client.request<Record<string, unknown>>("/server", {
    method: "POST",
    form: {
      cloud: args.cloud,
  ...
  if (!operationId && !serverId) {
    throw new Error(
      `Cloudways POST /server did not return operation_id or server id: ${JSON.stringify(body).slice(0, 300)}`,
    );
  }
  return { operation_id: operationId ?? "", server_id: serverId, raw: body };
}

function normalizeServer(raw: Record<string, unknown>): ServerSummary | null {
  ...
  const rawApps = Array.isArray(raw.apps) ? (raw.apps as unknown[]) : [];
  const apps: ServerAppSummary[] = [];
...
function pickServerId(body: Record<string, unknown>): string | undefined {
  const direct = pickString(body, ["server_id"]);
  if (direct) return direct;
  const server = body.server;
  if (server && typeof server === "object") {
    return pickString(server as Record<string, unknown>, ["id", "server_id"]);
  }
  return undefined;
}
