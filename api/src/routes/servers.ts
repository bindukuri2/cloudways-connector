...
import {
  ...
  listRegions,
  listServers,
} from "../cloudways/servers.js";
import type {
  CreateServerResponse,
  ServerOperationStatus,
} from "../types.js";

interface PendingServerOp {
  plannedLabel: string;
  startedAt: number;
  serverId?: string;
  lastStatus?: string;
...
export async function registerServerRoutes(
  app: FastifyInstance,
  deps: ServerRoutesDeps,
): Promise<void> {
  const { cloudways } = deps;

  const serverOperations = new Map<string, PendingServerOp>();
  ...
  app.get<{ Querystring: { cloud?: string } }>("/regions", async (request, reply) => {
  ...
  app.get<{ Querystring: { cloud?: string } }>("/server_sizes", async (request, reply) => {
    const cloud = request.query.cloud?.trim();
    if (!cloud) {
      reply.code(400);
  ...
  app.post("/servers", async (request, reply) => {
    ...
    if (existing) {
      request.log.info(
        { serverLabel: args.serverLabel, serverId: existing },
        "servers.create.reused",
      );
    ...
    request.log.info(
      { serverLabel: args.serverLabel, operationId, serverId: created.server_id },
      "servers.create.started",
    );
    ...
    async (request, reply) => {
      ...
      if (!pending) {
        ...
        try {
          ...
          const resp: ServerOperationStatus = {
            operationId: id,
