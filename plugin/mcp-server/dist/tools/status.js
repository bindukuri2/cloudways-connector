...
const inputShape = {
    ...
    waitMs: z
        ...
        .max(30_000)
        .optional()
        .describe("Optional client-side wait before polling, in milliseconds. Use ~5000 between repeated calls to avoid hammering the backend."),
};
...
function renderStatus(s) {
    ...
    if (s.cloudwaysAppId)
        lines.push(`Cloudways app ID: ${s.cloudwaysAppId}`);
    if (s.cloudwaysServerId)
        lines.push(`Cloudways server ID: ${s.cloudwaysServerId}`);
    ...
    if (s.cache) {
        lines.push(`Cache: app=${s.cache.appCachePurged ? "purged" : "not purged"}, varnish=${s.cache.varnishPurged ? "purged" : "not purged"}`);
        if (s.cache.appCacheError)
            lines.push(`App cache warning: ${s.cache.appCacheError}`);
        if (s.cache.varnishError)
            lines.push(`Varnish warning: ${s.cache.varnishError}`);
    }
    if (s.url)
