...
const inputShape = {
    workspaceRoot: z
        .string()
        .min(1)
        .describe("Absolute path to the user's workspace root. The config is written to <workspaceRoot>/.deploy-intel/config.json."),
    serverId: z
        .string()
        .min(1)
    ...
    appId: z
        ...
        .min(1)
        .optional()
        .describe("Optional Cloudways app id to remember, so future deploys skip both the server picker and the label-based app lookup."),
};
export function registerSaveServerSelection(server) {
    server.registerTool("save_server_selection", {
        ...
        try {
            ...
            if (serverLabel)
                next.cloudwaysServerLabel = serverLabel;
            if (appId)
                next.cloudwaysAppId = appId;
...
async function ensureGitignoreEntry(workspaceRoot) {
...
//# sourceMappingURL=save_server_selection.js.map