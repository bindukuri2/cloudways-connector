...
export interface ScaffoldArgs {
  ...
  /** Lowercase kebab-case theme slug; also used as the mu-plugin file name. */
  themeSlug: string;
  /** Human-readable theme name written into style.css. */
  themeName: string;
  /** Site title that the mu-plugin enforces. */
  siteTitle?: string;
  /** When true (default), removes index.html after extracting its content. */
  removeIndexHtml?: boolean;
  /** Author string written into the theme + mu-plugin headers. */
  author?: string;
}
...
export async function scaffoldWpTheme(args: ScaffoldArgs): Promise<ScaffoldResult> {
  if (!SLUG_RE.test(args.themeSlug)) {
    throw new Error(
      `Invalid theme slug "${args.themeSlug}": must be lowercase kebab-case (a-z, 0-9, -).`,
    );
  }
  ...
  if (!html) {
    throw new Error(
      `No index.html at ${indexHtml}. The static-landing scaffolder needs a top-level index.html to work from.`,
    );
  }
  ...
  if (args.removeIndexHtml !== false) {
    try {
      ...
      warnings.push(
        `Could not remove ${indexHtml}: ${err instanceof Error ? err.message : String(err)}. ` +
    ...
    warnings.push(
      `Left ${indexHtml} in place. Apache will serve it before WordPress's index.php — ` +
  ...
  for (const f of rootEntries) {
    if (f.endsWith(".html") && f !== "index.html") {
      warnings.push(
        `Found ${f} at workspace root. WordPress won't route through it; ` +
  ...
  if (await isFile(path.join(root, ".htaccess"))) {
    warnings.push(
      `Found .htaccess at workspace root. Cloudways manages its own .htaccess for WordPress; ` +
...
async function writeFile(p: string, contents: string, created: string[]): Promise<void> {
...
function splitHtml(html: string): SplitHtml {
...
function rewriteRelativeAssetUrls(html: string): string {
  return html.replace(
    ...
    (full, attr, quote, url) => {
      ...
      if (!v) return full;
      if (/^(?:https?:|\/\/|\/|#|mailto:|tel:|data:|<\?)/i.test(v)) return full;
...
function renderStyleCss(args: StyleArgs): string {
  return `/*
...
function renderHeaderPhp(headInner: string, bodyAttrs: string): string {
...
function renderFooterPhp(): string {
  return `<?php
/**
...
function renderFrontPagePhp(bodyInner: string): string {
  return `<?php
/**
...
function renderIndexPhp(): string {
  return `<?php
/**
...
function renderFunctionsPhp({ slug }: { slug: string }): string {
  return `<?php
/**
 * Theme bootstrap.
 */
...
interface MuPluginArgs {
  slug: string;
  siteTitle: string;
  author: string;
}

function renderMuPlugin(args: MuPluginArgs): string {
...
function stripHeadTags(headInner: string, _drop: string[]): string {
