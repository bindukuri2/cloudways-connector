...
import type { Evidence, ProjectStack, DetectedPlugin, DetectedTheme } from "../types.js";
 ...
 */
function candidateCoreRoots(root: string): string[] {
...
interface CoreProbe {
  score: number;
  hits: Evidence[];
  /** Which sub-root contained the WP install (used to resolve wp-content). */
  installRoot: string;
}

async function probeWordPressCore(root: string): Promise<CoreProbe> {
  let best: CoreProbe = { score: 0, hits: [], installRoot: root };
...
interface BedrockProbe {
  matched: boolean;
  hits: Evidence[];
}

async function probeBedrock(root: string, composer: ComposerJson | null): Promise<BedrockProbe> {
  const hits: Evidence[] = [];
  let matched = false;
...
async function readComposerJson(root: string): Promise<ComposerJson | null> {
...
async function probeWooCommerce(
  root: string,
  ...
  if (composer) {
    ...
    for (const dep of Object.keys(allDeps)) {
      if (
        dep === "wpackagist-plugin/woocommerce" ||
...
async function detectTheme(wpContent: string | null): Promise<DetectedTheme | undefined> {
  if (!wpContent) return undefined;
  const themesDir = path.join(wpContent, "themes");
  const entries = await listDir(themesDir);
  for (const slug of entries) {
    if (slug.startsWith(".")) continue;
...
async function detectPlugins(wpContent: string | null): Promise<DetectedPlugin[]> {
  if (!wpContent) return [];
  const pluginsDir = path.join(wpContent, "plugins");
  const out: DetectedPlugin[] = [];
  for (const slug of await listDir(pluginsDir)) {
    if (slug.startsWith(".") || slug === "index.php") continue;
...
async function detectPhpVersion(root: string, composer: ComposerJson | null): Promise<string> {
  const phpVersionFile = await readTextSafe(path.join(root, ".php-version"));
  if (phpVersionFile) {
    const v = phpVersionFile.trim();
    if (v) return normalizePhp(v);
  }
  const platformPhp = composer?.config?.platform?.php;
  if (platformPhp) return normalizePhp(platformPhp);

  const composerPhp = composer?.require?.["php"];
  if (composerPhp) return normalizePhp(composerPhp);
...
function normalizePhp(raw: string): string {
  const m = raw.match(/(\d+\.\d+)/);
  return m && m[1] ? m[1] : DEFAULT_PHP;
}

function resolveWpContent(installRoot: string | null): string | null {
 ...
 */
async function probeStaticLanding(root: string): Promise<{ matched: boolean; hits: Evidence[] }> {
  const hits: Evidence[] = [];
  const indexHtml = path.join(root, "index.html");
  if (!(await isFile(indexHtml))) return { matched: false, hits };

  hits.push({ signal: "static-html:index.html", path: indexHtml });

  for (const f of ["style.css", "main.css", "styles.css", "app.css"]) {
    const p = path.join(root, f);
    if (await isFile(p)) hits.push({ signal: `static-css:${f}`, path: p });
  }
  for (const f of ["main.js", "app.js", "script.js", "index.js"]) {
    const p = path.join(root, f);
    if (await isFile(p)) hits.push({ signal: `static-js:${f}`, path: p });
  }
  for (const d of ["assets", "images", "img", "static", "public", "css", "js"]) {
    const p = path.join(root, d);
    if (await isDir(p)) hits.push({ signal: `static-dir:${d}`, path: p });
  }
...
export async function detectProjectStack(workspaceRoot: string): Promise<ProjectStack> {
  ...
  if (composer) evidence.push({ signal: "composer.json", path: path.join(root, "composer.json") });
  ...
  let wpInstallRoot: string | null = null;
