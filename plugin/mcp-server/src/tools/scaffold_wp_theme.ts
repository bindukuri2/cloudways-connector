...
const inputShape = {
  workspaceRoot: z
    ...
    .min(1)
    .describe(
      "Absolute path to the user's workspace root. Must contain an index.html at the top level.",
    ),
  themeSlug: z
    .string()
    .min(2)
    ...
    .describe(
      "Lowercase kebab-case slug for the WordPress theme directory (e.g. 'havemyburger').",
    ),
  themeName: z
    .string()
    .min(1)
    .max(120)
    .describe(
      "Human-readable theme name written into style.css Theme Name header. Defaults to the slug if not provided.",
    ),
  siteTitle: z
    .string()
    .min(1)
    ...
    .optional()
    .describe(
      "Site title the activator mu-plugin will set via update_option('blogname', ...). Defaults to the <title> from index.html, then to themeName.",
    ),
  removeIndexHtml: z
    .boolean()
    .optional()
    .describe(
      "If true (default), deletes the original index.html after extracting its content. Apache's DirectoryIndex prefers index.html over index.php, so leaving it makes the static page win over WordPress.",
    ),
};

export function registerScaffoldWpTheme(server: McpServer): void {
  server.registerTool(
    "scaffold_wp_theme",
    {
