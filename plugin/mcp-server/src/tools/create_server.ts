...
const LABEL_MSG =
  "must be 3-60 chars, lowercase alphanumeric with single dashes as separators (e.g. 'my-wp-site')";
...
const inputShape = {
  ...
  serverLabel: z
    ...
    .describe(
      "Human label for the server (visible in the Cloudways dashboard). Must be 3-60 chars, lowercase alphanumeric with single dashes. Used for idempotency: calling create_server again with the same label after a timed-out attempt will reuse the in-flight/created server instead of billing a duplicate.",
    ),
  appLabel: z
    .string()
    .min(3)
    ...
    .describe(
      "Label for the initial app Cloudways creates alongside the server. Must be 3-60 chars, lowercase alphanumeric with single dashes. Set this to the same slug you will pass to `deploy` as appName so no second app has to be created afterward. Cloudways rejects labels shorter than 3 characters with HTTP 422.",
    ),
  projectName: z
    .string()
    .min(3)
  ...
  timeoutSeconds: z
    ...
    .optional()
    .describe(
      "Max seconds to wait for the server to reach a running state. Default 720s (12 minutes).",
    ),
...
export function registerCreateServer(server: McpServer): void {
  server.registerTool(
    "create_server",
    {
      ...
      try {
        ...
        if (!created.operationId) {
          return {
            ...
            structuredContent: { error: "no operation id" } as Record<string, unknown>,
          };
        }
