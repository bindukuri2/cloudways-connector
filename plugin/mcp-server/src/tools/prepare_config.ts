...
const inputShape = {
  ...
  appNameOverride: z
    ...
    .optional()
    .describe(
      "Optional explicit Cloudways app label. Defaults to the slugified basename of the workspace.",
    ),
  serverId: z
    .string()
    .min(1)
    .optional()
    .describe(
      "Optional Cloudways server id chosen by the skill's picker/create flow. Overrides any value cached in .deploy-intel/config.json.",
    ),
  existingAppIdOverride: z
    ...
    .optional()
    .describe(
      "Optional existing Cloudways app id to reuse instead of creating a new one. Overrides any value cached in .deploy-intel/config.json.",
    ),
};

interface WorkspaceConfig {
  cloudwaysServerId?: string;
  cloudwaysServerLabel?: string;
  cloudwaysAppId?: string;
}

export function registerPrepareConfig(server: McpServer): void {
  server.registerTool(
    "prepare_config",
    {
    ...
    async ({ workspaceRoot, appNameOverride, serverId, existingAppIdOverride }) => {
      ...
      if (resolvedServerId) request.serverId = resolvedServerId;
      if (resolvedAppId) request.existingAppId = resolvedAppId;

      const serverLine = resolvedServerId
        ? `Server: ${cached.cloudwaysServerLabel ? `${cached.cloudwaysServerLabel} (id=${resolvedServerId})` : resolvedServerId}${
            serverId ? " [override]" : cached.cloudwaysServerId ? " [from .deploy-intel/config.json]" : ""
          }`
...
async function readWorkspaceConfig(workspaceRoot: string): Promise<WorkspaceConfig> {
  ...
  try {
    ...
    if (typeof obj.cloudwaysServerId === "string") out.cloudwaysServerId = obj.cloudwaysServerId;
    if (typeof obj.cloudwaysServerLabel === "string")
      out.cloudwaysServerLabel = obj.cloudwaysServerLabel;
    if (typeof obj.cloudwaysAppId === "string") out.cloudwaysAppId = obj.cloudwaysAppId;
    return out;
  } catch {
    return {};
  }
