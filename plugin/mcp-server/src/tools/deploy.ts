...
const inputShape = {
  appName: z
    ...
    .regex(
      /^[a-z0-9]+(-[a-z0-9]+)*$/,
      "appName must be 3-60 chars, lowercase alphanumeric with single dashes as separators (e.g. 'my-wp-site'). Cloudways rejects shorter labels with HTTP 422.",
    )
    .describe(
      "Cloudways app label. Must be 3-60 chars, lowercase alphanumeric with single dashes (e.g. 'my-wp-site'). Normally derived by prepare_config from the workspace folder name.",
    ),
  appType: z
  ...
  serverId: z
    ...
    .optional()
    .describe(
      "Cloudways server id to deploy into. Normally forwarded from prepare_config (which reads .deploy-intel/config.json). Required unless the backend has a CLOUDWAYS_SERVER_ID fallback in its .env.",
    ),
};
